...
describe("DELETE /api/configuraciones/:configuracionId", () => {
    ...
    it("debería devolver 400 si configuracionId no es válido", async() => {
        const res = await request(app)
        .delete(`/api/configuraciones/1234`)
        .set("Authorization", `Bearer ${admin.token}`);
    ...
    it("debería devolver 404 si la configuración no existe", async() => {
        const res = await request(app)
        .delete(`/api/configuraciones/999999999999999999999999`)
        .set("Authorization", `Bearer ${admin.token}`);
