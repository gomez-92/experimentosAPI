...
describe("PUT /api/configuraciones/:configuracionId", () => {
    ...
    it("debería devolver 200 y los datos de la configuración actualizada", async() => {
        const res = await request(app)
        .put(`/api/configuraciones/${configuracion._id}`)
        .set("Authorization", `Bearer ${sistema.token}`)
        .send({ valorMin: 50 });
        assert.strictEqual(res.status, 200);
        assert.ok(res.body.payload);
        assert.strictEqual(res.body.payload._id, configuracion._id);
        assert.strictEqual(res.body.payload.valorMin, 50);
    });
    
    it("debería devolver 400 si configuracionId es inválido", async() => {
        const res = await request(app)
        .put(`/api/configuraciones/1234`)
        .set("Authorization", `Bearer ${sistema.token}`)
        .send({ valorMin: 50 });
    ...
    it("debería devolver 404 si la configuración no existe", async() => {
        const res = await request(app)
        .put(`/api/configuraciones/999999999999999999999999`)
        .set("Authorization", `Bearer ${sistema.token}`)
        .send({ valorMin: 50 });
