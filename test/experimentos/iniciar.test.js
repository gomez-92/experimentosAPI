...
describe("POST /api/experimentos/:experimentoId/iniciar", () => {
    ...
    before(async() => {
        ...
        .send({ nombre: "Moke iniciar", duracion: 10000 });
    ...
    it("debería devolver 404: experimento no encontrado", async() => {
        const res = await request(app)
        .post(`/api/experimentos/123412341234123412341234/iniciar`)
        .set("Authorization", `Bearer ${sistema.token}`);
    ...
    it("debería devolver 403 porque iniciar es funcion de sistema", async() => {
        const res = await request(app)
        .post(`/api/experimentos/${experimento._id}/iniciar`)
        .set("Authorization", `Bearer ${admin.token}`);
        assert.strictEqual(res.status, 403);
    ...
    it("debería devolver 200 con el estado del experimento en iniciado", async() => {
        const res = await request(app)
        .post(`/api/experimentos/${experimento._id}/iniciar`)
        .set("Authorization", `Bearer ${sistema.token}`);
        ...
        assert.strictEqual(res.body.payload.estado, "EN_CURSO");
    });
