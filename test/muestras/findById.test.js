...
describe("GET /api/muestras/:muestraId", () => {
    ...
    it("debería devolver 401 si no está autenticado", async() => {
        ...
        .get(`/api/muestras/${muestra._id}`);
        assert.strictEqual(res.status, 401);
    });

    it("debería devolver 404 si la muestra no existe", async() => {
        const res = await request(app)
        .get(`/api/muestras/999999999999999999999999`)
        .set("Authorization", `Bearer ${admin.token}`);
    ...
    it("debería devolver 200 y los datos de la muestra", async() => {
        const res = await request(app)
        .get(`/api/muestras/${muestra._id}`)
        .set("Authorization", `Bearer ${admin.token}`);
