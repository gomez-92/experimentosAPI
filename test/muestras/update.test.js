...
describe("PUT /api/muestras/:muestraId", () => {


    let experimento, registro, muestra, admin, sistema;
    ...
    it("debería devolver 400 si muestraId es inválido", async() => {
        const res = await request(app)
        .put(`/api/muestras/1234`)
        .set("Authorization", `Bearer ${admin.token}`)
        .send({ valor: 25 });
        assert.strictEqual(res.status, 400);
    });

    it("debería devovler 404 si la muestra no existe", async() => {
        const res = await request(app)
        .put(`/api/muestras/999999999999999999999999`)
        .set("Authorization", `Bearer ${admin.token}`)
        .send({ valor: 25 });
    ...
    it("debería devolver 200 y los datos de la muestra actualizados", async() => {
        const res = await request(app)
        .put(`/api/muestras/${muestra._id}`)
        .set("Authorization", `Bearer ${admin.token}`)
        .send({ valor: 25 });
