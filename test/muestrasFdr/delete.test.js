...
describe("DELETE /api/experimentos/:experimentoId/muestraFDR", () => {
    ...
    before(async() => {
        ...
        .send({ nombre: "Moke FDR", duracion: 10000 });
    ...
    it("debería devolver 400 si muestraId es inválido", async() => {
        const res = await request(app)
        .delete(`/api/muestrasFDR/1234`)
        .set("Authorization", `Bearer ${admin.token}`);
    ...
    it("debería devolver 403 si no esta autorizado", async() => {
        const res = await request(app)
        .delete(`/api/muestrasFDR/${muestraFDR._id}`)
        .set("Authorization", `Bearer ${sistema.token}`);
    ...
    it("debería devolver 404 si la muestraFDR no existe", async() => {
        const res = await request(app)
        .delete(`/api/muestrasFDR/999999999999999999999999`)
        .set("Authorization", `Bearer ${admin.token}`);
        assert.strictEqual(res.status, 404);
    });

    it("debería devolver 200 y los datos de la muestraFDR eliminada", async() => {
        const res = await request(app)
        .delete(`/api/muestrasFDR/${muestraFDR._id}`)
        .set("Authorization", `Bearer ${admin.token}`);
