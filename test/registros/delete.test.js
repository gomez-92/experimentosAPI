...
describe("DELETE /api/registros/:registroId", () => {
    ...
    before(async() => {
        ...
        .set("Authorization", `Bearer ${sistema.token}`)
        .send({ nombre: "Moke delete reg", tipo: "temperatura", unidad: "ºC" });
    ...
    it("deberia devolver 400 si el registroId es no válido", async() => {
        const res = await request(app)
        .delete(`/api/registros/1234`)
        .set("Authorization", `Bearer ${admin.token}`);
    ...
    it("debería devolver 404 si el registro no existe", async() => {
        const res = await request(app)
        .delete(`/api/registros/999999999999999999999999`)
        .set("Authorization", `Bearer ${admin.token}`);
        assert.strictEqual(res.status, 404);
    });

    it("debería devolver 401 si no esta autenticado", async() => {
        const res = await request(app)
        .delete(`/api/registros/${registro._id}`);
    ...
    it("debería devolver 200 y los datos del registro eliminado", async() => {
        const res = await request(app)
        .delete(`/api/registros/${registro._id}`)
        .set("Authorization", `Bearer ${admin.token}`);
